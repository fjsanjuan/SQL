CREATE PROCEDURE dbo.xpEdoWebUtiliad(
		@Empresa			  varchar(5), 
		@Periodo			  int,
		@Ejercicio		  int,
...
BEGIN
  ...
  IF EXISTS(SELECT * FROM cta WHERE Cuenta = @CtaVenta)
    EXEC spReporteadorSaldo @Empresa, @Periodo, @Ejercicio, @CtaVenta, @TipoReporte, NULL, @Venta OUTPUT 
  ELSE 
    SELECT @Venta = 0
  IF EXISTS(SELECT * FROM cta WHERE Cuenta = @CtaDescuento)
    EXEC spReporteadorSaldo @Empresa, @Periodo, @Ejercicio, @CtaCosto, @TipoReporte, NULL, @Costo OUTPUT 
  ELSE 
    SELECT @Costo = 0
  IF EXISTS(SELECT * FROM cta WHERE Cuenta = @CtaDevolucion)
    EXEC spReporteadorSaldo @Empresa, @Periodo, @Ejercicio, @CtaDescuento, @TipoReporte, NULL, @Descuento OUTPUT 
  ELSE 
    SELECT @Descuento = 0
  IF EXISTS(SELECT * FROM cta WHERE Cuenta = @CtaCosto)
    EXEC spReporteadorSaldo @Empresa, @Periodo, @Ejercicio, @CtaDevolucion, @TipoReporte, NULL, @Devolucion OUTPUT 
  ELSE 
    SELECT @Devolucion = 0
