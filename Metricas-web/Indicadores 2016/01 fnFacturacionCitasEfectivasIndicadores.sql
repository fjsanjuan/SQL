--/* SOPDESA */ EXEC xpFacturacionCitasEfectivas 32, 'GMCHE', 0, '01/12/2015', '30/12/2015'

--alter PROCEDURE xpFacturacionCitasEfectivasIndicadores 
IF EXISTS(SELECT * FROM SYSOBJECTS WHERE ID = OBJECT_ID('fnFacturacionCitasEfectivasIndicadores') AND TYPE = 'FN')
	DROP FUNCTION fnFacturacionCitasEfectivasIndicadores
GO
...
BEGIN    
  ...
  SELECT Vc.Id, IdOrden = Vs.Id, IdFactura = VF.Id, IdNota = NULL, CancelacionId=VF.CancelacionId,           
   ...
   WHERE VF.FechaConclusion = @FechaA  
     ...
     AND VF.Mov IN ('FEL Garantía', 'FEL Seguro', 'FEL Servicio')                      
