...
SELECT DISTINCT     
       ...
       AE  = CONVERT(FLOAT, (SELECT ISNULL(SUM(Cantidad),0) FROM CCAcumProspec22015 A  
                              WHERE A.Usuario = I.UsuCC   
                                ...
                                AND A.Subtipo IN ('Retención', 'Promoción'))),            
       AL  = CONVERT(FLOAT, (SELECT COUNT(*) FROM CCVistaProspeccion P--CCProspectosD D JOIN CCProspectos  C ON C.idProspeccion = D.idProspeccion            
                              WHERE P.Usuario = I.UsuCC   
                                AND dbo.fnFechaSinHora(P.dtAContactar) = I.Fecha             
                                AND P.Tipo = 'Servicio'   
                                AND P.Subtipo = 'LeadIT'   
                                AND P.IDEvento = 1)            
                          + (SELECT COUNT(*) FROM CCHistProspectosD D JOIN CCHistProspectos  C ON C.idProspeccionH = D.idProspeccionH            
      ...
      --2015                         
       AP=0,
       AR   = CONVERT(FLOAT,(SELECT COUNT(*) FROM CCVistaProspeccion P--CCProspectosD D JOIN CCProspectos  C ON C.idProspeccion = D.idProspeccion            
                              WHERE P.Usuario = I.UsuCC   
                                AND dbo.fnFechaSinHora(P.dtAContactar) = I.Fecha             
                                AND P.Tipo = 'Servicio'   
       ...
       AV   = CONVERT(FLOAT,(SELECT COUNT(*)   
                               FROM (SELECT 'A'=P.IdProspeccion ,'B'=dbo.fnFechaSinHora(P.dtAContactar)            
                                   FROM CCVistaProspeccion P--CCProspectosD D JOIN CCProspectos  C ON C.idProspeccion = D.idProspeccion            
                                  WHERE P.Usuario = I.UsuCC   
                                    AND dbo.fnFechaSinHora(P.dtAContactar) = I.Fecha   
                                    AND P.Tipo = 'Servicio'   
             ...
             + (SELECT COUNT(*)     
                            FROM (SELECT 'A'=D.IdProspeccionH ,'B'=dbo.fnFechaSinHora(D.dtAContactar)             
                                   FROM CCHistProspectosD D JOIN CCHistProspectos  C ON C.IdProspeccionH = D.IdProspeccionH            
                                  WHERE C.Usuario = I.UsuCC   
                                    AND dbo.fnFechaSinHora(D.dtAContactar) = I.Fecha   
                                    AND C.Tipo = 'Servicio'   
...
***********************************************************************************************************/
      ...
      CJ = CONVERT(FLOAT,(SELECT ISNULL(SUM(Cantidad),0) FROM CCAcumProspec42015 A  
                           WHERE A.Usuario = I.UsuCC   
							...
							WHERE A.Usuario = I.UsuCC
							AND A.FechaMin = I.Fecha))),            
      CK = CONVERT(FLOAT,(SELECT COUNT(*) FROM CCProspectosD D 
							JOIN CCProspectos  C ON C.idProspeccion = D.idProspeccion            
                           WHERE C.Usuario = I.UsuCC   
							...
							WHERE A.Usuario = I.UsuCC
							AND A.FechaMin = I.Fecha))            
                       + (SELECT COUNT(*) FROM CCHistProspectosD D 
							JOIN CCHistProspectos  C ON C.idProspeccionH = D.idProspeccionH            
                           WHERE C.Usuario = I.UsuCC   
                             AND dbo.fnFechaSinHora(D.dtContacto) = I.Fecha             
                             AND C.Tipo = 'Servicio'   
							...
							WHERE A.Usuario = I.UsuCC
							AND A.FechaMin = I.Fecha))),            
      CM = CONVERT(FLOAT,(SELECT COUNT(*) FROM CCProspectosD D JOIN CCProspectos  C ON C.idProspeccion = D.idProspeccion            
                           WHERE C.Usuario = I.UsuCC   
                             AND dbo.fnFechaSinHora(D.dtAContactar) = I.Fecha             
                             AND C.Tipo = 'Servicio'   
      ...
      CP = CONVERT(FLOAT,(SELECT COUNT(*) FROM CCProspectosD D JOIN CCProspectos  C ON C.idProspeccion = D.idProspeccion            
                           WHERE C.Usuario = I.UsuCC   
                             AND dbo.fnFechaSinHora(D.dtAContactar) = I.Fecha             
                             AND C.Tipo = 'Nuevos'   
                             AND D.Subtipo = 'Encuesta'   
